...
## #1 The Overlay Pattern (Root > Backdrop + Container)
...
selector form, really simplifies matters, like this:
...
for mobile one for desktop. On the [bearjam.dev](/) site we also re-use this
again in the footer.
...
you can use the `children` prop, and in our use case here it actually makes a
lot of sense.

First let's look at how it looks to use it in the simplest case. Here's our code
...
We get to pass a function, in this case, the function for the link component.
Magic things that are happening:

- Access to `href`, `label` and `active` as function parameters
- This function gets mapped onto a link array

Let's have a look at the implementation code that makes this pattern possible:
...
And here we are again consuming our `<Nav />` component for the mobile aspect of
our header:
